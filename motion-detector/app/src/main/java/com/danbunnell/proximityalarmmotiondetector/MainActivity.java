...
import com.danbunnell.proximityalarmmotiondetector.ble.BLEDevice;
import com.danbunnell.proximityalarmmotiondetector.ble.BLEListener;
import com.danbunnell.proximityalarmmotiondetector.ble.BLEUtil;
import com.danbunnell.proximityalarmmotiondetector.camera.CameraSourcePreview;
import com.danbunnell.proximityalarmmotiondetector.camera.GraphicOverlay;

/**
 * Connects to a client Bluetooth device and sends x-position information from a face captured
 * using the Android Vision API's FaceDetector.  Also, receives subject distance information from
 * the Bluetooth client and displays it for the face being tracked.
 */
public class MainActivity extends AppCompatActivity implements BLEListener{

    private static final int COMMAND_BYTE = 0;
    private static final int FACE_X_POSITION_BYTE = 4;

    private static final byte CMD_SEND_FACE_X = 0x01;

    private static final byte CMD_CLIENT_NOTIFY_DISTANCE = 0x01;

    private static final String TAG = "ProximityAlarm";
    ...
    private final String TARGET_BLE_DEVICE_NAME = "zLaM39a";

    // Subject distance in cm
    ...
    private class FaceTracker extends Tracker<Face> {
        ...
        public void onUpdate(FaceDetector.Detections<Face> detectionResults, Face face) {
            ...
            mFaceGraphic.updateFace(face, distance);
            ...
            byte[] buf = new byte[5]; // 5-byte initialization

            // CSE590 Student TODO:
            // Write code that puts in your data into the buffer
            buf[COMMAND_BYTE] = CMD_SEND_FACE_X;
            buf[FACE_X_POSITION_BYTE] = getFaceX(face);
    ...
    public void onBleDataReceived(byte[] data) {
        ...
        switch(command) {
            case CMD_CLIENT_NOTIFY_DISTANCE:
                int distance = getWord(upper, lower);
                Log.i(TAG, "Distance received: " + String.format("%dcm", distance));
                updateDistance(distance);
                break;
            default:
                Log.w(TAG, String.format("Unknown command received: %x", command));
        }
    }
    ...
    /**
     * Updates the distance of the camera subject.
     *
     * @param d a distance in cm
     */
    private void updateDistance(int d) {
        distance = d;
    }

    /**
     * Gets the X-position of the specified face.
     *
     * @param face the face
     * @return     an x-position
     */
    private static byte getFaceX(Face face) {
        float xPosition = face.getPosition().x;

        // Face can be detected past the edge of the screen
        if (xPosition < 0) {
            xPosition = 0;
        } else if (xPosition > CAMERA_PREVIEW_HEIGHT) {
            xPosition = CAMERA_PREVIEW_HEIGHT;
        }

        // Reverse direction
        xPosition = CAMERA_PREVIEW_HEIGHT - xPosition;

        return (byte) (Math.round(xPosition  / CAMERA_PREVIEW_WIDTH * 255) & 0xFF);
    }

    /**
     * Converts two bytes into a 16-bit word.
     *
     * @param upper upper byte
     * @param lower lower byte
     * @return      a 16-bit word
     */
    private static int getWord(byte upper, byte lower) {
